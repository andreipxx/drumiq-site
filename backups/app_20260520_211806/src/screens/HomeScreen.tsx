  ...
  return (
    ...
    >
      ...
      {stats && stats.offersCount > 0 && (() => {
        ...
        const isCritical = acceptRate < 70;
        const warningColor = isCritical ? colors.stop : colors.think;
        const warningBg = isCritical ? colors.stopBg : colors.thinkBg;
        if (acceptRate < 75) return (
          <View style={[s.warningCard, { backgroundColor: warningBg, borderColor: warningColor }]}>
            <View style={s.warningRow}>
              <Text style={[s.warningIcon, { color: warningColor }]}>{isCritical ? '🚨' : '⚠️'}</Text>
              <View style={{ flex: 1 }}>
                <Text style={[s.warningTitle, { color: warningColor }]}>
                  Rata acceptare: {acceptRate}%
                </Text>
                <Text style={[s.warningDesc, { color: warningColor }]}>
                  {isCritical ? 'SUB PRAG BLOCARE 70%!' : 'Aproape de prag 70%'}
                </Text>
              </View>
            </View>
          </View>
          ...
          return (
            <View key={r.id} style={[s.rideCard, { backgroundColor: colors.surface, borderColor: colors.border, borderLeftColor: v.color }]}>
              ...
              <View style={s.rideMiddle}>
                <Text style={[s.ridePrice, { color: colors.text }]}>{r.grossEarnings.toFixed(2)}<Text style={[s.ridePriceUnit, { color: colors.textMuted }]}> lei</Text></Text>
...
const s = StyleSheet.create({
  ...
  statCard: { width: '48%', borderWidth: 1, borderRadius: 14, padding: 14 },
  ...
  // Warning card
  warningCard: { padding: 14, borderRadius: 14, borderWidth: 1, marginBottom: 14 },
  warningRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  warningIcon: { fontSize: 20 },
  warningTitle: { fontSize: 13, fontWeight: '900', fontFamily: 'monospace' },
  warningDesc: { fontSize: 11, fontWeight: '600', marginTop: 2 },
