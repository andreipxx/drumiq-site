  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: colors.bg }]}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <TouchableOpacity
          onPress={() => {
            Alert.alert('Sterge toate datele?', 'Cursele, statisticile si cache-ul vor fi sterse permanent.', [
              { text: 'Anuleaza', style: 'cancel' },
              { text: 'Sterge tot', style: 'destructive', onPress: async () => {
                await clearAllRides();
                Alert.alert('Gata', 'Toate datele au fost sterse.');
              }},
            ]);
          }}
          style={[s.btnSecondary, { borderColor: colors.critic, marginTop: 10 }]}
          activeOpacity={0.7}
        >
          <Text style={[s.btnSecondaryText, { color: colors.critic }]}>STERGE TOATE DATELE</Text>
        </TouchableOpacity>
        ...
        <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          {debugEvents.length === 0 ? (
            <Text style={[s.hint, { color: colors.textTertiary }]}>Nicio activitate. Deschide Bolt si asteapta o oferta.</Text>
          ) : (
