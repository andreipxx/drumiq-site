...
class DPOverlayService : Service() {
    ...
    companion object {
        ...
        const val EXTRA_NET_SHORT = "netShort"   // "+5.2" or "-1.3" for circle
    ...
    private fun applyData(intent: Intent) {
        ...
        val netShort = intent.getStringExtra(EXTRA_NET_SHORT) ?: ""
        val baseSymbol = when (verdict) {
            "stop"  -> "✗"
            "think" -> "?"
            "go"    -> "$"
            else    -> "·"
        }
        val symbol = if (netShort.isNotEmpty()) "$baseSymbol $netShort" else baseSymbol
