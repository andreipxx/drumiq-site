...
// SEC-HIGH-5: Device whitelist from environment variable instead of hardcoded
const WHITELISTED_DEVICES = new Set(
  (Deno.env.get('WHITELISTED_DEVICES') ?? '').split(',').map(s => s.trim()).filter(Boolean)
);
...
Deno.serve(async (req: Request) => {
  ...
  // SEC-HIGH-3: Rate limit trial activation attempts
  ...
  // SEC-CRIT-2: Return the trial code so the client can store it for 24h refresh verification
  return jsonResponse({ token, plan: 'trial', existing: false, code });
});
