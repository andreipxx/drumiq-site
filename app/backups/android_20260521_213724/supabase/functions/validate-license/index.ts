 ...
 *
 * TODO(audit-v2): Add verify_jwt = false in Supabase Dashboard function config
 * so the anon key Bearer token isn't validated as a user JWT by the gateway.
 */
...
// SEC-M4: CORS Allow-Origin: * is acceptable here — this endpoint is called from
// a React Native mobile app (no browser Origin header). Restricting origins would
// not add security for mobile clients and would break web-based testing tools.
const CORS_HEADERS = {
...
// ─── Rate Limiting ────────────────────────────────────────────
// TODO(audit-v2): For true atomicity, migrate to a Postgres function:
//   CREATE FUNCTION check_rate_limit_atomic(p_ip TEXT, p_max INT, p_window_ms BIGINT)
//   RETURNS BOOLEAN AS $$ ... INSERT ON CONFLICT DO UPDATE ... RETURNING attempt_count $$
// and call via supabase.rpc('check_rate_limit_atomic', {...}).
// Current implementation minimizes the TOCTOU window with upsert but is not fully atomic.
...
async function checkRateLimit(
  ...
  // Single read to get current state
  ...
  if (!data) {
    // New IP — insert with count=1 (upsert handles concurrent first-requests)
  ...
  // Increment — use conditional update to reduce TOCTOU window:
  // only update if attempt_count hasn't changed since our read
  const { data: updated, error: updateErr } = await supabase
    ...
    .eq('ip_address', ip)
    .eq('attempt_count', data.attempt_count)
  ...
  if (updateErr || !updated) {
    ...
    const { data: recheck } = await supabase
      ...
      .eq('ip_address', ip)
      .single();
    return (recheck?.attempt_count ?? 0) < RATE_LIMIT_MAX;
  }
...
Deno.serve(async (req: Request) => {
  ...
  // SEC-HIGH-2: Prioritize cf-connecting-ip (set by Cloudflare, not spoofable) over x-forwarded-for
