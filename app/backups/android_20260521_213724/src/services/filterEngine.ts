...
export function saveThresholds(t: UnifiedThresholds): Promise<void> {
  return withSaveLock(async () => {
    ...
    const count = enabledFlags.filter(f => t[f]).length;
    if (count > 1) {
      ...
      // The newly enabled flag is one that's true in t but false (or absent) in prev
      const newlyEnabled = prev
        ? enabledFlags.find(f => t[f] && !prev![f])
        : null;
      // Keep only the newly enabled flag; fall back to the first enabled flag
      const keepFlag = newlyEnabled || enabledFlags.find(f => t[f]);
      for (const f of enabledFlags) {
        (t as any)[f] = f === keepFlag;
      }
    }
