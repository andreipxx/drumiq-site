...
export async function getDeviceId(): Promise<string> {
  ...
  if (!id) {
    if (typeof globalThis.crypto?.getRandomValues === 'function') {
      id = 'fb-' + Array.from(crypto.getRandomValues(new Uint8Array(8))).map(b => b.toString(16).padStart(2, '0')).join('');
    } else {
      id = 'fb-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 10);
    }
  }
...
async function migrateLegacyLicense(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log('Legacy license migrated to JWT successfully');
    } catch (e) {
      ...
      console.warn('Legacy migration server call failed, user may need to re-activate:', e);
    }
...
async function refreshJwtIfNeeded(currentPayload: LicenseJwtPayload): Promise<void> {
  ...
  try {
    ...
    const deviceId = await getDeviceId();
    const newToken = await callValidateLicense(storedCode, deviceId);
    ...
    if (err instanceof LicenseServerError && REVOCATION_CODES.includes(err.errorCode)) {
      console.warn('License revoked by server during refresh:', err.errorCode, err.message);
      await deleteStoredJwt();
