...
// Valid plan values (must match PlanTier union in ../types)
const VALID_PLANS: readonly string[] = ['trial', 'pro', 'root'];

function decodeJwtPayload(token: string): LicenseJwtPayload | null {
  try {
    // Structural check: JWT must have exactly 3 parts (header.payload.signature)
    ...
    // so the client can verify with a public key. The structural checks below catch
    // malformed/garbage payloads but cannot prevent a forged JWT with valid structure.

    // Structural validation: required fields must exist and have correct types
    if (typeof payload.sub !== 'string' || payload.sub.length === 0) return null;
    if (typeof payload.exp !== 'number' || !isFinite(payload.exp) || payload.exp <= 0) return null;
    if (typeof payload.plan !== 'string' || !VALID_PLANS.includes(payload.plan)) return null;
...
export async function getLicenseState(): Promise<LicenseState> {
  ...
  const license = jwtPayloadToLicense(payload);
  const uid = await getCurrentUserId();
  const ridesUsed = await readRideCounter(uid);
...
// The checksum below prevents casual editing of the stored value but is NOT
// cryptographically secure — a determined attacker can reverse it.

// Embedded salt for ride counter checksum (obfuscation, not cryptographic security)
const _RC_SALT = 'diq$r1d3#2026';

/** Derive a simple checksum from counter value + salt */
function _rideChecksum(count: number): string {
  // Simple hash: sum of char codes of "salt:count" mod a large prime, hex-encoded
  const input = `${_RC_SALT}:${count}`;
  let hash = 0;
...
/** Write ride counter with tamper-detection checksum */
async function writeRideCounter(uid: string | null, count: number): Promise<void> {
  const key = userKey(RIDE_COUNTER_BASE, uid);
  const value = `${count}:${_rideChecksum(count)}`;
  await AsyncStorage.setItem(key, value);
}

/** Read ride counter, verifying checksum. Returns 0 on tamper or missing data. */
async function readRideCounter(uid: string | null): Promise<number> {
  const key = userKey(RIDE_COUNTER_BASE, uid);
  const raw = await AsyncStorage.getItem(key);
  if (!raw) return 0;

  // Backward compat: legacy format is just a plain integer string
  if (!raw.includes(':')) {
    const legacy = parseInt(raw, 10) || 0;
    // Rewrite in new format
    await writeRideCounter(uid, legacy);
    return legacy;
  }

  const sepIdx = raw.lastIndexOf(':');
  const countStr = raw.substring(0, sepIdx);
  const storedHash = raw.substring(sepIdx + 1);
  const count = parseInt(countStr, 10);

  if (isNaN(count) || count < 0) {
    if (__DEV__) console.warn('Ride counter: invalid format, resetting to 0');
    await writeRideCounter(uid, 0);
    return 0;
  }

  if (storedHash !== _rideChecksum(count)) {
    if (__DEV__) console.warn('Ride counter: checksum mismatch (possible tampering), resetting to 0');
    await writeRideCounter(uid, 0);
    return 0;
  }
...
export async function incrementRideCounter(): Promise<number> {
  const uid = await getCurrentUserId();
  const cur = await readRideCounter(uid);
  const next = cur + 1;
  await writeRideCounter(uid, next);
