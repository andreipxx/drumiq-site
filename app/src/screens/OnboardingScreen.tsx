...
export default function OnboardingScreen({ onComplete }: Props) {
  ...
  if (step === 'splash') {
    return (
      <View style={[s.center, { backgroundColor: colors.bg }]}>
        ...
        <Text style={[s.brand, { color: colors.cyan, fontFamily: fontsLoaded ? FONT.displayXB : FONT.system }]}>DRUMIQ</Text>
        <Text style={[s.tagline, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>Știi instant dacă merită cursa</Text>
      </View>
    );
  }
  ...
  const dots = dotIndex >= 0 ? (
    <View style={s.dotsRow}>
      {DOT_STEPS.map((_, i) => (
        <View key={i} style={[s.dot, { backgroundColor: i <= dotIndex ? colors.cyan : colors.border }]} />
      ))}
    </View>
  ) : null;
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg }}>
        ...
        <View style={[s.center, { backgroundColor: 'transparent' }]}>
          ...
          <Text style={[s.stepTitle, { color: colors.cyan, fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>Hai să configurăm DRUMIQ</Text>
          <View style={{ height: 32 }} />
          <View style={s.bulletList}>
            <BulletRow color={colors.go} symbol="$" text="GO = cursă profitabilă" textColor={colors.text} fontsLoaded={fontsLoaded} />
            <BulletRow color={colors.think} symbol="?" text="THINK = analizează" textColor={colors.text} fontsLoaded={fontsLoaded} />
            <BulletRow color={colors.stop} symbol="X" text="STOP = evită" textColor={colors.text} fontsLoaded={fontsLoaded} />
          </View>
          <TouchableOpacity onPress={doGeoCheck} activeOpacity={0.7} style={{ width: '100%', marginTop: 28 }}>
            <LinearGradient colors={colors.gradButton} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.btn, { borderRadius: RADIUS.md, marginTop: 0 }]}>
              <Text style={[s.btnTxt, { fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>{'CONTINUĂ →'}</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      </View>
  ...
  if (step === 'geofence') {
    return (
      <View style={[s.center, { backgroundColor: colors.bg }]}>
        ...
        <Text style={[s.hint, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>DRUMIQ verifică dacă te afli într-un oraș disponibil.</Text>
      </View>
    );
  }
  ...
  if (step === 'geofence_fail') {
    ...
    return (
      <View style={[s.center, { backgroundColor: colors.bg }]}>
        ...
        <Text style={[s.hint, { color: colors.go, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>Disponibil în: Baia Mare, România</Text>
        <TouchableOpacity onPress={doGeoCheck} activeOpacity={0.7} style={{ width: '100%', marginTop: 28 }}>
          <LinearGradient colors={colors.gradButton} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.btn, { borderRadius: RADIUS.md, marginTop: 0 }]}>
            <Text style={[s.btnTxt, { fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>VERIFICĂ DIN NOU</Text>
          </LinearGradient>
        </TouchableOpacity>
        ...
        {__DEV__ && (
          <TouchableOpacity onPress={() => setStep('car')} activeOpacity={0.6}>
            <Text style={[s.skipLink, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>{'Continuă oricum (demo) →'}</Text>
          </TouchableOpacity>
        )}
      </View>
    );
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 24, paddingTop: 48 }}>
          ...
          <Text style={[s.hint, { color: colors.textMuted, textAlign: 'center', marginBottom: 16, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>
          ...
          <TouchableOpacity onPress={() => setStep('permissions')} activeOpacity={0.6} style={{ marginTop: 20, alignItems: 'center' }}>
            <Text style={[s.skipLink, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>{'Altă mașină (setez manual) →'}</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
    );
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg }}>
        ...
        <View style={[s.center, { backgroundColor: 'transparent' }]}>
          ...
          <Text style={[s.hint, { color: colors.textMuted, marginBottom: 24, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>
            DRUMIQ are nevoie de aceste permisiuni ca să funcționeze corect
          </Text>

          <View style={[s.privacyBox, { borderColor: colors.go, marginBottom: 20 }]}>
            <Text style={[s.privacyTxt, { color: colors.go, fontFamily: fontsLoaded ? FONT.bodySB : FONT.system }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            style={{ width: '100%', marginTop: 28, opacity: allPermsOk ? 1 : 0.5 }}
          >
            <LinearGradient colors={colors.gradButton} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.btn, { borderRadius: RADIUS.md, marginTop: 0 }]}>
              <Text style={[s.btnTxt, { fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>{allPermsOk ? 'CONTINUĂ →' : 'CONTINUĂ ORICUM →'}</Text>
            </LinearGradient>
          </TouchableOpacity>

          {!allPermsOk && (
            <Text style={[s.hint, { color: colors.textMuted, fontSize: 12, marginTop: 8, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg }}>
        ...
        <View style={[s.center, { backgroundColor: 'transparent' }]}>
          {dots}
          <Text style={{ fontSize: 48, marginBottom: 16 }}>{'🎉'}</Text>
          <Text style={[s.stepTitle, { color: colors.cyan, fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>Gata!</Text>
          <View style={{ height: 16 }} />
          <View style={[s.trialBox, { backgroundColor: colors.bgCard, borderColor: colors.cyan + '40' }]}>
            <Text style={[s.trialTitle, { color: colors.text, fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>Trial activ</Text>
            <Text style={[s.trialDetail, { color: colors.cyan, fontFamily: fontsLoaded ? FONT.mono : FONT.systemMono }]}>
              {TRIAL.RIDES} curse · {TRIAL.DAYS} zile
            </Text>
          </View>
          <TouchableOpacity onPress={() => setStep('bolt')} activeOpacity={0.7} style={{ width: '100%', marginTop: 28 }}>
            <LinearGradient colors={colors.gradButton} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.btn, { borderRadius: RADIUS.md, marginTop: 0 }]}>
              <Text style={[s.btnTxt, { fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>{'CONTINUĂ →'}</Text>
            </LinearGradient>
          </TouchableOpacity>
  ...
  if (step === 'bolt') {
    return (
      <View style={[s.center, { backgroundColor: colors.bg }]}>
        ...
        <TouchableOpacity onPress={openBolt} activeOpacity={0.7} style={{ width: '100%', marginTop: 28 }}>
          <LinearGradient colors={colors.gradSuccess} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.btn, { borderRadius: RADIUS.md, marginTop: 0 }]}>
            <Text style={[s.btnTxt, { color: '#fff', fontFamily: fontsLoaded ? FONT.display : FONT.system }]}>DESCHIDE BOLT DRIVER</Text>
          </LinearGradient>
        </TouchableOpacity>
        <TouchableOpacity onPress={finish} activeOpacity={0.6}>
          <Text style={[s.skipLink, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>{'Închide configurarea →'}</Text>
        </TouchableOpacity>
      </View>
    );
  }
...
function PermRow({ label, desc, checked, onPress, colors, fontsLoaded }: {
  label: string; desc: string; checked: boolean; onPress: () => void; colors: any; fontsLoaded: boolean;
}) {
  return (
    <TouchableOpacity
      style={[s.permRow, {
        backgroundColor: colors.bgCard,
        borderColor: checked ? colors.cyan + '50' : colors.border,
      }]}
      onPress={checked ? undefined : onPress}
      activeOpacity={checked ? 1 : 0.7}
    >
      <View style={[s.permCheck, { backgroundColor: checked ? colors.cyan : 'transparent', borderColor: checked ? colors.cyan : colors.border }]}>
        {checked && <Text style={s.permCheckmark}>✓</Text>}
      </View>
      <View style={{ flex: 1 }}>
        <Text style={[s.permLabel, { color: checked ? colors.cyan : colors.text, fontFamily: fontsLoaded ? FONT.bodySB : FONT.system }]}>{label}</Text>
        <Text style={[s.permDesc, { color: colors.textMuted, fontFamily: fontsLoaded ? FONT.body : FONT.system }]}>{desc}</Text>
      </View>
      {!checked && <Text style={{ color: colors.cyan, fontSize: 13, fontWeight: '700', fontFamily: fontsLoaded ? FONT.bodySB : FONT.system }}>ACORDĂ</Text>}
    </TouchableOpacity>
  );
}

function BulletRow({ color, symbol, text, textColor, fontsLoaded }: {
  color: string; symbol: string; text: string; textColor: string; fontsLoaded: boolean;
}) {
  return (
    <View style={s.bulletRow}>
      <View style={[s.bulletCircle, { backgroundColor: color }]}>
        <Text style={[s.bulletSymbol, { fontFamily: fontsLoaded ? FONT.displayXB : FONT.system }]}>{symbol}</Text>
      </View>
      <Text style={[s.bulletText, { color: textColor, fontFamily: fontsLoaded ? FONT.bodySB : FONT.system }]}>{text}</Text>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  btnTxt: { color: '#fff', fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  trialBox: {
    borderWidth: 1, borderRadius: RADIUS.md, padding: 20, width: '100%', alignItems: 'center',
  },
