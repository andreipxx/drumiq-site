...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, RefreshControl, Alert, Platform, AppState } from 'react-native';
...
// LOW-13: extracted to deduplicate ride rendering (was copy-pasted in dayRides and rides maps)
function RideItem({ r, colors }: { r: Ride; colors: any }) {
  const v = VERDICT_DISPLAY[r.verdict];
  const dt = new Date(r.timestamp);
  const timeStr = `${dt.getHours().toString().padStart(2, '0')}:${dt.getMinutes().toString().padStart(2, '0')}`;
  return (
    <View style={[st.ride, { backgroundColor: colors.surface, borderColor: colors.border, borderLeftColor: v.color }]}>
      <View style={st.rideTop}>
        <Text style={[st.rideTime, { color: colors.textMuted }]}>{timeStr}</Text>
        <Text style={[st.ridePrice, { color: colors.text }]}>{(r.grossEarnings ?? 0).toFixed(2)} lei</Text>
      </View>
...
export default function TrackerScreen() {
  ...
  return (
    ...
    >
      ...
      {/* Daily goal — only show for today period */}
      ...
      <View style={st.statGrid}>
        ...
        <View style={[st.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[st.statVal, { color: colors.text }]}>
            {(stats?.distanceKm ?? 0).toFixed(0)}<Text style={[st.statUnit, { color: colors.textMuted }]}> km</Text>
          </Text>
        </View>
        <View style={[st.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[st.statLbl, { color: colors.textMuted }]}>MEDIE LEI/KM</Text>
          <Text style={[st.statVal, { color: colors.go }]}>{(stats?.avgPpkm ?? 0).toFixed(2)}</Text>
        </View>
        <View style={[st.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[st.statLbl, { color: colors.textMuted }]}>MEDIE LEI/MIN</Text>
          <Text style={[st.statVal, { color: (stats?.avgPpmin ?? 0) >= 0.50 ? colors.go : colors.think }]}>{(stats?.avgPpmin ?? 0).toFixed(2)}</Text>
        </View>
      </View>
      ...
      {selectedDay && dayStats && (
        <>
          ...
          {dayRides.length === 0 ? (
            ...
            dayRides.map((r) => <RideItem key={r.id} r={r} colors={colors} />)
          )}
        </>
      )}
      ...
      {!selectedDay && rides.length === 0 ? (
        ...
        rides.map((r) => <RideItem key={r.id} r={r} colors={colors} />)
      ) : null}
